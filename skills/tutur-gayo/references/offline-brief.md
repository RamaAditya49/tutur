# Offline Brief

## Local Facts

- Purpose: support source-aware generation and review for Bahasa Gayo.
- Region/context: dataran tinggi Aceh, terutama komunitas Gayo.
- ISO 639-3: `gay`.
- Glottocode: `gayo1244`.
- Source confidence: Level 2 because KBDA Daring and public Gayo-Indonesian dictionary material exist.
- Bahasa Gayo is separate from Bahasa Aceh and should not borrow Acehnese forms by default.

## Guardrails

- Do not invent vocabulary.
- Do not assume one Gayo variety represents all speakers.
- Do not borrow Acehnese forms just because the language is used in Aceh Province.
- Preserve Indonesian terms for modern technical vocabulary unless a verified Gayo term is available.
- For public, ritual, legal, educational, or official use, recommend native-speaker or Balai Bahasa review.

## Sample Prompts

- "Use $tutur-gayo to review this Bahasa Gayo output."
- "Make a source-aware Gayo version of this short community message."
- "Check whether this is Bahasa Gayo or Indonesian with Gayo words."

## Safe Output Patterns

- Ask for target community/dialect when the audience is specific.
- Produce in Indonesian first when source confidence is limited.
- Translate only verified short phrases.
- Mark uncertain terms for KBDA or speaker review.

## Local Example

Request:

> Buat teks pendek untuk mengajak anak muda menjaga bahasa Gayo.

Safe response shape:

> Buat baseline Indonesia, lalu pakai padanan Gayo hanya jika dapat diverifikasi dari KBDA/Kamus Gayo atau sampel penutur.

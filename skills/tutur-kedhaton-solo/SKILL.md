...
1. Identify the target.
   - If the user says "Keraton Solo", map it to Surakarta/Kasunanan Surakarta.
   - If the user says "Mangkunegaran", do not assume it is identical to Kasunanan Surakarta. Mention that it is a related Surakarta court context and keep the output conservative.
   - If the text is Indonesian, adapt meaning into a court-inspired Javanese/Indonesian hybrid only as far as the request allows.
...
3. Apply Surakarta Kedhaton signals.
   ...
   - Use passive forms such as `dipun-` and suffixes such as `-aken` when forming formal Javanese phrases.
   - Use Kedhaton pronouns only when speaker status is clear. If status is unclear, avoid first/second-person pronoun-heavy phrasing.
   - Use special vocabulary only from known references or common krama/Kedhaton forms.
...
Basa Kedhaton Surakarta has role-sensitive pronoun choices. Do not freely swap them as decoration.

Read `references/style-guide.md` before using pronouns. When unsure, rewrite to avoid direct "I/you" pronouns.
